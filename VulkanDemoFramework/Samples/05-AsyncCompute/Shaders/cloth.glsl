...
struct PhysicsVertex {
    ...
    vec3 velocity;
    float mass;
    vec3 force; // TODO: maybe we can remove this
    uint joints[ 12 ];
};
...
// NOTE: we can't use vec3 for positions and normals, it will pad each entry and
...
void main() {
    ...
    // NOTE: uncomment these to make the cloth behave like a sail
    ...
    for ( uint s = 0; s < sim_steps; ++s ) {
        ...
        for ( uint v = current_thread; v < vertex_count; v+= GROUP_SIZE ) {
            if ( ( physics_vertices[ v ].start_position == fixed_vertex_1 ) || ( physics_vertices[ v ].start_position == fixed_vertex_2 )
                // NOTE: uncomment these lines to make the cloth behave like a sail
